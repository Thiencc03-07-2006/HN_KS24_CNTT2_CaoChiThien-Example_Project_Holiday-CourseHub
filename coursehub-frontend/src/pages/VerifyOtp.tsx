...
const VerifyOtp: React.FC = () => {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', background: 'var(--background-color)' }}>
      <Card style={{ width: 400, borderRadius: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.3)', border: 'none', background: '#fff' }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          <Title level={2} style={{ color: '#1e293b', margin: 0 }}>Xác thực tài khoản</Title>
        ...
        <Form layout="vertical" onFinish={onFinish} initialValues={{ email: defaultEmail }} requiredMark={false} noValidate>
          <Form.Item
            label={<span style={{ color: '#334155' }}>Email đăng ký</span>}
            name="email"
            rules={[
          ...
          <Form.Item
            label={<span style={{ color: '#334155' }}>Mã OTP (6 chữ số)</span>}
            name="otpCode"
            rules={[
          ...
          >
            <Input size="large" maxLength={6} placeholder="123456" style={{ letterSpacing: '8px', textAlign: 'center' }} />
          </Form.Item>
